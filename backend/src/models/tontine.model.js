...
const tontineSchema = Joi.object({
  ...
  members: Joi.array().items(Joi.string()).default([]), // Liste des membres (par défaut vide)
  admin: Joi.array().items(Joi.string()).default([]), // Liste des admins (par défaut vide)
  invite: Joi.array().items(Joi.string()).default([]), // Liste des utilisateurs invités à réjoindre la tontine (par défaut vide)
